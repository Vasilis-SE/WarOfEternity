...
public class MagePlayerService implements PlayerInterface {

    private static final int STARTING_STRENGTH = 5;
    private static final int STARTING_AGILITY = 9;
    private static final int STARTING_INTELLIGENCE = 14;

    @Override
    public PlayerClassesEnum getPlayerClass() {
        return PlayerClassesEnum.MAGE;
    }
    ...
    public int calculateDamage(PlayerModel player, int equippedItemDamage) {
        return (int) ((equippedItemDamage * 0.3) + (player.getIntelligence() * 0.6) + (player.getLevel() * 0.1));
    }
    ...
    public void applyLevelUpAttributeGrowth(PlayerModel player, PlayerAttributeStatsModel startingStats, PlayerAttributeStatsModel itemAttributeBonuses) {
        ...
        player.setIntelligence(startingStats.getIntelligence() + itemAttributeBonuses.getIntelligence() + ((level * 2) - 2));
    }
}
