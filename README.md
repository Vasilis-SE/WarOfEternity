...
> go north
> travel south-east
The game also supports the use of only the noun part on direction commands, so you can also use : north, south, north-east etc. to travel to the appropriate areaModel.
...
Sail Commands
     ...
     With the sail commands the player can travel from an areaModel to another through a ship. There are multiple dock's in the areaModel that the player can travel with providing
...
can be the name of the destination areaModel. So a sail command can be like :
...
with the help of the local healer and talk with people to get you to an areaModel via ship. The commands that can be use in this section are : buy, sell, talk, purchase, bargain.
...
pick, take, grab, search, snatch, use, open, equip. With the verbs pick, take, grab and snatch the player can be items that are layimg  around the areaModel that he are in.
With the search verb the player can search the areaModel that he is in to find items that are laying  around. With the use <item> command the player can use various items
...
the merchants which sell and buy items, healers with which you replenish your lost health and sailors / captains to travel you to areaModels that are reachable only
...
the command talk to sailor / captain you get as the result the name of the areaModel that the ship captain can take you to and the amount of gold required to make
the trip.
...
of the game and every location (game areaModel) as a pin point. Also on the map there are yellow dashed lines which represent the direction the player must take
in order to travel from one areaModel to another.
...
(the button with the sound icon). There are two types of sound files that are played in this game the sounds of the areaModels and the sounds of battle.